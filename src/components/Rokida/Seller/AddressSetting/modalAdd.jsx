...
class modal extends Component {
  ...
  addAddress = async () => {
    ...
    if (this.state.required_name === false && this.state.required_phone === false && this.state.required_detail_address === false) {
      ...
      await addressStore.addAddress(addressInfo)
      .then(async (res) => {
        if (res.data === false) {
          await this.setState({
            loader: false
  ...
  render() {
    return (
      ...
      {this.state.loader === true ? <Loader /> : ''}
        ...
        <Modal.Footer>
          <Button onClick={this.props.hideModel} id="cancel-add-address" variant="outline-dark">
            Hủy
        </Button>
          <Button onClick={this.addAddress} variant="warning">
            Lưu
        </Button>
        </Modal.Footer>
      </>
