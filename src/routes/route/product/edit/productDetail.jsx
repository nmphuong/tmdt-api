...
class productDetail extends Component {
  ...
  setValueDefault = async () => {
    var categoryBS = {
      category1: this.state.data.cate_id[2].name,
      category2: this.state.data.cate_id[1].name,
      category3: this.state.data.cate_id[0].name
    }
    var classify1Value = [], classify2Value = [], classify1Key = [], classify2Key = [], classify1Price = [], classify2Price = [], classify1Warehouse = [], classify2Warehouse = [], classify1Sku = [], classify2Sku = []
    for (var i = 0; i < this.state.data.classify.length; i++) {
      if (classify1Key.length === 0) {
        classify1Key.push(this.state.data.classify[i].classify_key)
        classify1Value.push(this.state.data.classify[i].classify_value)
        classify1Price.push(this.state.data.classify[i].classify_price)
        classify1Warehouse.push(this.state.data.classify[i].classify_warehouse)
        classify1Sku.push(this.state.data.classify[i].classify_sku)
      } else {
        if (classify1Key[0] === this.state.data.classify[i].classify_key) {
          classify1Key.push(this.state.data.classify[i].classify_key)
          classify1Value.push(this.state.data.classify[i].classify_value)
          classify1Price.push(this.state.data.classify[i].classify_price)
          classify1Warehouse.push(this.state.data.classify[i].classify_warehouse)
          ...
          classify2Key.push(this.state.data.classify[i].classify_key)
          classify2Value.push(this.state.data.classify[i].classify_value)
          classify2Price.push(this.state.data.classify[i].classify_price)
          classify2Warehouse.push(this.state.data.classify[i].classify_warehouse)
          classify2Sku.push(this.state.data.classify[i].classify_sku)
        }
      }
    }
    await this.setState({
      nameProd: this.state.data.name,
      description: this.state.data.description,
      category: this.state.data.cate_id[0].id,
      categoryBasic: categoryBS,
      branch: this.state.data.metaProduct.branch,
      material: this.state.data.metaProduct.material,
      origin: this.state.data.metaProduct.origin,
      showClassify: (this.state.data.classify.length === 0) ? false : true,
      classify1Value: classify1Value,
      classify2Value: classify2Value,
      classify1Key: classify1Key,
      classify2Key: classify2Key,
      classify1Price: classify1Price,
      classify2Price: classify2Price,
      classify1Warehouse: classify1Warehouse,
      classify2Warehouse: classify2Warehouse,
      classify1Sku: classify1Sku,
      classify2Sku: classify2Sku
    })
  }
              ...
              <div className="row p-0 m-0 position-relative">
                <div className="contain col-md-9 col-lg-10">
                  ...
                  <BasicInformation
                    ...
                    nameProdBasic={this.state.nameProd}
                    descriptionBasic={this.state.description}
                    categoryBasic={this.state.categoryBasic}
                    branchBasic={this.state.branch}
                    materialBasic={this.state.material}
                    originBasic={this.state.origin}
                  />
                  {/* Seller infomation */}
                  <SalesInformation
                    ...
                    showClassifyBasic={this.state.showClassify}
                    classify1Value={this.state.classify1Value}
                    classify2Value={this.state.classify2Value}
                    classify1Key={this.state.classify1Key}
                    classify2Key={this.state.classify2Key}
                    classify1Price={this.state.classify1Price}
                    classify2Price={this.state.classify2Price}
                    classify1Warehouse={this.state.classify1Warehouse}
                    classify2Warehouse={this.state.classify2Warehouse}
                    classify1Sku={this.state.classify1Sku}
                    classify2Sku={this.state.classify2Sku}
                  />
                  {/* Manager image */}
                  <ManagerImage
